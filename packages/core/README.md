...
countryAt(2.1734, 41.3851);   // "ES" - Barcelona, though France's rectangle reaches it
countryAt(-30, 45);           // undefined - mid-Atlantic
```

Its own entry point: about 230 kB of Natural Earth 1:50m outlines, quantised to roughly
1.1 km, which the main entry never touches. It exists because every service publishes the
bounding rectangle of the area it covers and no country is a rectangle, so extents overlap
across borders and the catalogue cannot tell whose ground a point is on. Register it with
`setCountryResolver` from `@orthogea/catalog`, or pass it to `createMosaic({ countryAt })`.

`undefined` means "cannot say", not "nowhere": callers are expected to do nothing rather
than exclude something. Good for choosing between national services; not for deciding which
side of a border a parcel is on.

The table is generated by `scripts/build-boundaries.mjs` and committed, so building the
package needs no network. Re-run it when Natural Earth publishes a new edition.

## Coverage helpers

```ts
layerCoversPoint(layer, 11.2558, 43.7696);   // honours `coverage` when the record narrows its hull
layerAreaSqKm(layer);
