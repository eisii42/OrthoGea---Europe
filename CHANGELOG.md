...
- **`coverage`, an optional list of boxes narrowing a record's advertised extent.** A service
  ...
  The cross-border half of the problem is solved by the country outlines below; `coverage`
  remains the remedy for the regional half.

- **`@orthogea/core/boundaries`, which answers what the extents cannot: the country a coordinate
  is actually in.** With it registered, `bestOrthophotoFor` stops handing Barcelona to France,
  Vienna to Czechia and Copenhagen to Sweden. Over the 41 test cities it moves the function from
  32 correct to 38; the four that remain are regional overlaps inside Italy, which no country
  outline can settle.
  ...
  `createMosaic({ countryAt })` takes the same resolver, so the map and the single-record call
  agree. A per-call `countryAt` option overrides the registered one, and `countryAt: false`
  disables it.

  It is a separate entry point because the outlines are about 230 kB, and the main
  `@orthogea/core` entry - the one on the drawing path of every map - stays at 13 kB and does not
  reference them. The data is Natural Earth 1:50m, public domain, quantised to about 1.1 km and
  delta encoded, generated by `packages/core/scripts/build-boundaries.mjs` and committed so the
  build needs no network.

  1:110m was measured first and is six times smaller, but it places Geneva in France - the Swiss
  enclave is below its resolution - and at 1:50m no coordinate tried returned a *wrong* country,
  only "cannot say". That distinction decided it: the resolver is treated as advisory, and
  `undefined` leaves the ranking untouched, so silence is harmless where a confident mistake
  would send a map to the wrong national service. A 5 km coastal tolerance stops a city on a
  lagoon or a strait - Venice, Lisbon, Copenhagen, Stockholm - from reading as being at sea.

  Where a country is known and has no catalogued source, `bestOrthophotoFor` now returns the
  European base rather than the neighbour whose rectangle reaches in: 2 m of the right place
  beats 20 cm of the wrong one. `imageryStackFor` keeps the neighbour, behind the base, because a
  stack is a list of things to try.
...
- **`bestOrthophotoFor` is pinned to real coordinates.** The test asserted only that a point in
  ...
  cities against the country and region that should serve them, with the remaining known failures
